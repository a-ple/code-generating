CLASS zcl_email_notification DEFINITION
  ...
  PUBLIC SECTION.
    ...
    " Method to send email with optional attachment
    METHODS send_email
      IMPORTING
  ...
  PRIVATE SECTION.
    " Prepare recipients from DB or test constant
...
CLASS zcl_email_notification IMPLEMENTATION.
  ...
  METHOD send_email.
    ...
    " Use provided recipients or fallback to test email
    ...
    " Create mail message with HTML body
  ...
  METHOD prepare_recipients.
    " Placeholder: select active recipients from DB custom table
    SELECT email_address FROM zemail_notification_recipients
  ...
  METHOD log_email_status.
    ...
    " Example logging - replace with proper log implementation
    ...
    IF iv_message IS NOT INITIAL.
      cl_demo_output=>write( |Message: { iv_message }| ).
    ENDIF.
  ENDMETHOD.
